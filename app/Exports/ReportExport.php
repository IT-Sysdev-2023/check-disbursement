...
class ReportExport implements FromQuery, WithHeadings
{
    ...
    public function query()
    {
        ...
        if (in_array('status', $columns)) { //select the 'check_forwarded_statuses' status if there is a relationship there otherwise use the 
            $columns = array_map(
                fn($col) => $col === 'status'
                ? DB::raw('COALESCE(check_forwarded_status.status, check_statuses.status) AS status')
                : $col,
                $columns
            );
        }
        ...
        return CheckStatus::select($columns)
            ...
            ->where([['check_statuses.checkable_type', 'cv'], ['borrowed_checks.checkable_type', 'cv']])
            ->when(
                !empty($validated['status']),
                fn($query) =>
                $query->whereIn(DB::raw('COALESCE(check_forwarded_status.status, check_statuses.status)'), $validated['status'])
            )
            ->when(
                !empty($validated['bu']),
                fn($query) =>
