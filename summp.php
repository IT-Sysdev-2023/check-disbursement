    ...
    public function handle(): void
    {
        ...
        for ($i = 0; $i < count($files); $i += 2) {
            ...
            try {
                ...
                $bytes = Storage::disk('cheque_share')->get($image);

                $payload = self::setPayLoad($bytes);

                $response = self::setResponse($payload);
                ...
                // ScannedRecordEvent::dispatch($result, $this->user->id);
                ...
                //     AlreadyScannedEvent::dispatch($alreadyScanned, $this->user->id);
...
- Return valid JSON only, with no leading or trailing text.";
    ...
    private function setPayLoad($bytes)
    {
        return [
            'contents' => [
                [
    ...
    private function  setResponse($payload)
    {
